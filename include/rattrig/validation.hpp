...
template <typename T>
constexpr bool are_collinear(const std::pair<T, T>& p_1,
                             const std::pair<T, T>& p_2,
                             const std::pair<T, T>& p_3) noexcept {
    return cross(
        std::pair<T, T>(p_2.first - p_1.first, p_2.second - p_1.second),
        std::pair<T, T>(p_3.first - p_1.first, p_3.second - p_1.second)) == detail::zero<T>();
}

/// @brief Check if three points form a valid (non-degenerate) triangle.
template <typename T>
constexpr bool is_valid_triangle(const std::pair<T, T>& p_1,
                                 const std::pair<T, T>& p_2,
                                 const std::pair<T, T>& p_3) noexcept {
...
template <typename T>
constexpr bool is_valid_quadrance(const T& q) noexcept {
    return q >= detail::zero<T>();
}

/// @brief Check if a spread value is valid (in [0, 1]).
template <typename T>
constexpr bool is_valid_spread(const T& s) noexcept {
    return s >= detail::zero<T>() && s <= detail::one<T>();
}

/// @brief Check the triangle inequality for three quadrances.
template <typename T>
constexpr bool satisfies_triangle_inequality(const T& q_1, const T& q_2,
...
template <typename T>
constexpr bool is_acute_triangle(const T& s_1, const T& s_2,
                                 const T& s_3) noexcept {
    return s_1 < detail::one<T>() && s_2 < detail::one<T>() && s_3 < detail::one<T>();
}

/// @brief Check if a triangle is right-angled (one spread = 1).
template <typename T>
constexpr bool is_right_triangle(const T& s_1, const T& s_2,
                                 const T& s_3) noexcept {
    return s_1 == detail::one<T>() || s_2 == detail::one<T>() || s_3 == detail::one<T>();
}
...
template <typename T>
constexpr bool is_obtuse_triangle(const T& s_1, const T& s_2,
                                  const T& s_3) noexcept {
...
constexpr bool are_lines_parallel(const std::tuple<T, T, T>& l_1,
    ...
    return std::get<0>(l_1) * std::get<1>(l_2)
         - std::get<1>(l_1) * std::get<0>(l_2) == detail::zero<T>();
}
...
constexpr bool are_lines_perpendicular(const std::tuple<T, T, T>& l_1,
                                       const std::tuple<T, T, T>& l_2) noexcept {
    return std::get<0>(l_1) * std::get<0>(l_2)
         + std::get<1>(l_1) * std::get<1>(l_2) == detail::zero<T>();
}
...
/// @f]
template <typename T>
constexpr bool point_on_line(const std::pair<T, T>& point,
                             const std::tuple<T, T, T>& line) noexcept {
    T result = std::get<0>(line) * point.first
             + std::get<1>(line) * point.second
             + std::get<2>(line);
...
template <typename T>
constexpr bool point_in_triangle(const std::pair<T, T>& point,
