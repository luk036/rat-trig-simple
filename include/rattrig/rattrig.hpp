...
namespace rattrig {
    ...
    namespace detail {
        ...
        template <typename T, typename = void> struct has_static_zero : std::false_type {};
        template <typename T> struct has_static_zero<T, std::void_t<decltype(T::zero())>>
            : std::true_type {};

        template <typename T, typename = void> struct has_static_one : std::false_type {};
        template <typename T> struct has_static_one<T, std::void_t<decltype(T::one())>>
            : std::true_type {};

        /// @brief Return the zero constant for type T.
        template <typename T> constexpr T zero() noexcept {
        ...
        template <typename T> constexpr T one() noexcept {
        ...
        template <typename T> constexpr T four() noexcept {
            return one<T>() + one<T>() + one<T>() + one<T>();
        }

        /// @brief Return a pair of zeros.
        template <typename T> constexpr std::pair<T, T> zero_pair() noexcept {
            return std::pair<T, T>(zero<T>(), zero<T>());
        }

        /// @brief Return a tuple of three zeros.
        template <typename T> constexpr std::tuple<T, T, T> zero_triple() noexcept {
    ...
    template <typename T>
    constexpr T quadrance(const std::pair<T, T>& p_1, const std::pair<T, T>& p_2) noexcept {
    ...
    /// @return The spread
    template <typename T>
    constexpr T spread(const std::pair<T, T>& v_1, const std::pair<T, T>& v_2) noexcept {
    ...
    template <typename T>
    constexpr T cross(const std::pair<T, T>& v_1, const std::pair<T, T>& v_2) noexcept {
    ...
    /// @return The quadrance
    template <typename T> constexpr T quadrance_from_line(const std::pair<T, T>& p,
                                                          const std::tuple<T, T, T>& l) noexcept {
        T temp = std::get<0>(l) * p.first + std::get<1>(l) * p.second + std::get<2>(l);
        return temp * temp
               / quadrance(std::pair<T, T>(std::get<0>(l), std::get<1>(l)), detail::zero_pair<T>());
    }
    ...
    /// @return The spread
    template <typename T> constexpr T spread_from_line(const std::tuple<T, T, T>& l_1,
    ...
    /// @param[in] l_2 Second line as (a, b, c)
    /// @return The cross product
    template <typename T> constexpr T cross_from_line(const std::tuple<T, T, T>& l_1,
                                                      const std::tuple<T, T, T>& l_2) noexcept {
    ...
    template <typename T>
    constexpr std::tuple<T, T, T> quadrance_from_three_points(const std::pair<T, T>& p_1,
                                                              const std::pair<T, T>& p_2,
                                                              const std::pair<T, T>& p_3) noexcept {
        return std::make_tuple(quadrance(p_2, p_3), quadrance(p_1, p_3), quadrance(p_1, p_2));
    }
    ...
    template <typename T>
    constexpr std::tuple<T, T, T> spread_from_three_points(const std::pair<T, T>& p_1,
                                                           const std::pair<T, T>& p_2,
                                                           const std::pair<T, T>& p_3) noexcept {
    ...
    /// @tparam T A numeric type
    /// @return The cross product
    template <typename T> constexpr T cross_from_three_points(const std::pair<T, T>& p_1,
                                                              const std::pair<T, T>& p_2,
                                                              const std::pair<T, T>& p_3) noexcept {
    ...
    template <typename T> constexpr T quadrance3d(const std::tuple<T, T, T>& p_1,
    ...
    template <typename T>
    constexpr T spread3d(const std::tuple<T, T, T>& v_1, const std::tuple<T, T, T>& v_2) noexcept {
        T dot = std::get<0>(v_1) * std::get<0>(v_2) + std::get<1>(v_1) * std::get<1>(v_2)
    ...
    template <typename T> constexpr T twist(const std::pair<T, T>& p_1, const std::pair<T, T>& p_2,
                                            const std::pair<T, T>& p_3) noexcept {
    ...
    template <typename T> constexpr std::pair<T, bool> turn(const std::pair<T, T>& p_1,
                                                            const std::pair<T, T>& p_2,
                                                            const std::pair<T, T>& p_3) noexcept {
    ...
    template <typename T>
    constexpr T dilatation(const std::pair<T, T>& v_1, const std::pair<T, T>& v_2) noexcept {
        T q_1 = quadrance(v_1, detail::zero_pair<T>());
        T q_2 = quadrance(v_2, detail::zero_pair<T>());
        return q_2 / q_1;
    }
    ...
    template <typename T> constexpr T sine_law_product(const T& q, const T& s) noexcept {
        return q * s;
    }
    ...
    constexpr int quadrance_i32(std::pair<int, int> p_1, std::pair<int, int> p_2) noexcept {
    ...
    constexpr int spread_i32(std::pair<int, int> v_1, std::pair<int, int> v_2) noexcept {
    ...
    constexpr int cross_i32(std::pair<int, int> v_1, std::pair<int, int> v_2) noexcept {
        return v_1.first * v_2.second - v_1.second * v_2.first;
    }
    ...
    constexpr long long archimedes_i64(long long q_1, long long q_2, long long q_3) noexcept {
