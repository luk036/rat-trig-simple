...
struct Point2D {
    ...
    static constexpr Point2D new_(const T& x_, const T& y_) noexcept {
...
struct Point3D {
    ...
    constexpr Point3D() noexcept : x(detail::zero<T>()), y(detail::zero<T>()), z(detail::zero<T>()) {}
    constexpr Point3D(const T& x_, const T& y_, const T& z_) noexcept
        : x(x_), y(y_), z(z_) {}
...
struct Vector3D {
    ...
    constexpr Vector3D() noexcept : x(detail::zero<T>()), y(detail::zero<T>()), z(detail::zero<T>()) {}
    constexpr Vector3D(const T& x_, const T& y_, const T& z_) noexcept
        : x(x_), y(y_), z(z_) {}
...
struct Line2D {
    ...
    constexpr Line2D() noexcept : a(detail::zero<T>()), b(detail::zero<T>()), c(detail::zero<T>()) {}
    constexpr Line2D(const T& a_, const T& b_, const T& c_) noexcept
        : a(a_), b(b_), c(c_) {}
...
struct Triangle3D {
    ...
    /// @brief Calculate the three quadrances of the triangle sides.
    constexpr std::tuple<T, T, T> quadrances() const noexcept {
        T q1 = (p2.x - p3.x) * (p2.x - p3.x)
             + (p2.y - p3.y) * (p2.y - p3.y)
             + (p2.z - p3.z) * (p2.z - p3.z);
        T q2 = (p1.x - p3.x) * (p1.x - p3.x)
             + (p1.y - p3.y) * (p1.y - p3.y)
             + (p1.z - p3.z) * (p1.z - p3.z);
        T q3 = (p1.x - p2.x) * (p1.x - p2.x)
             + (p1.y - p2.y) * (p1.y - p2.y)
             + (p1.z - p2.z) * (p1.z - p2.z);
        return std::make_tuple(q1, q2, q3);
    }
